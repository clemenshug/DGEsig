...
# Filter to only include pertubatios that are in the top 6 of any query
R3 <- R_only_genetic %>%
  dplyr::filter(
    pert_iname %in% {
      group_by(., lspci_id) %>%
    ...
    # pert_iname %in% {
    #   group_by(., pert_iname) %>%
    #     # summarize(inequality = ineq::RS(tau), .groups = "drop") %>%
    #     # summarize(inequality = ineq::Theil(tau), .groups = "drop") %>%
    #     summarize(inequality = IQR(tau)) %>%
...
dist_fun <- function(x) dist(x, "euclidian")
